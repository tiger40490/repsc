/* Unrelated: I admire peers' efforts and nlg (not status) on
* wellness
* investment
* zbs, portable GTD, not localSys
* parenting

todo: refactor after it works. No need to follow the paradime of finding p2u->reshuffle

This solution is based on iterative.cpp
...
int main() {
  ...
  set<char> letters(tmp.begin(), tmp.end());
  L = letters.size();
  assert(L == tmp.size() && "tmp string has dupes");
  assert(C <= L);
  vector<char> v(tmp.begin(), tmp.begin()+C); //lowest combo
  dump(v);
  return 0;
  for(; calls<9992; ){
        if (!next_combo(v)){
...
}/* req: For L letters printed on a single button, after pressing C times, how many possible combinations? Better list them in ascending ordee
*/