...
//    ss1<<"inside DecOrderParser::parse"<<std::endl;
    auto * msg = cast<DecOrderMsg>(buf);
    ss2<<"Looking up the orders table using order id = "<<msg->oid<<" ..\n";
    if (Parser::orders.count(msg->oid) == 0){
      ss3<<"order not found. CancelOrder message dropped\n";
      return 'm'; //missing
    }
    Order& order = Parser::orders[msg->oid];
    if (order.qty < msg->qty){
      ss3<<"Current order qty "<<order.qty<<" is less than cancel qty "<<msg->qty<<" ! Will zero out order qty.\n";
      order.qty = 0;
      Parser::record("qDecOver#" + std::to_string(msg->oid), order.qty, order.stock);
    }else{
      order.qty -= msg->qty;
      ss3<<"order qty reduced to "<<order.qty<<std::endl;
