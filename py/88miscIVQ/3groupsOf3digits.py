...
# showcase: default return value is None, which evaluates to False 
import sys
...
def check(number1, myset): # a well-defined function
    number2 = 2*number1
...
winners=list()
nine_digits = tuple(range(1,10)) #prevent accidental change
for digit1 in (1,2,3):
    last8 = list(nine_digits) #make a copy
    last8.remove(digit1)
    for digit2 in last8:
        last7 = list(last8)
        last7.remove(digit2)
